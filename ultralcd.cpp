...
static void lcd_move_z()
{
    if (encoderPosition != 0)
    {
        ...
        if (max_software_endstops && current_position[Z_AXIS] > (Z_MAX_POS + Zcorrect))
            current_position[Z_AXIS] = (Z_MAX_POS + Zcorrect);
        encoderPosition = 0;
        #ifdef DELTA
        calculate_delta(current_position);
...
static void lcd_modify_z_max()
{
		...
		if (encoderPosition != 0)
    		{	
				...
				if (Zcorrect>0.00) Zcorrect=0.00;
		...
		if (lcdDrawUpdate)
    		{
        		...
        		lcd_implementation_drawedit(PSTR("Z home "), ftostr52(max_pos[2] + Zcorrect));
    		}
    		if (LCD_CLICKED)
    		{
